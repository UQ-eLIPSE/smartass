...
<table width="100%">
	...
	<tr>
		<td class="header" width="10px">
			...
			<input id="addCall" style="width:125" type="submit" class="button" name="_eventId_addCall" value="Add CALL"/><br><hr>
		...
		<td valign="top" colspan="2">
			<table border=0 frame=0 cellpadding=0>
				...
				<c:forEach items="${template.rows}" var="t">
					...
					</td><td><table border=0 frame=0 cellspacing=0><tr>
					<td bgcolor="#f0f0ff" width=<c:out value="${t.level * 10}"/>></td>
					<td>
						<c:set var="pos" value="${pos+1}"/> 
						<c:if test="${t.kind == 'text'}"><pre></c:if><c:out value="${t.html}"/><c:if test="${t.kind == 'text'}"></pre></c:if>
					</td>
					</tr></table></td></tr>
				</c:forEach>
			</table>
